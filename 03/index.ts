...
const isSimbol = (letter: string) => {
  if (letter === '.' || !isNaN(+letter)) return false
  return true
}

const checkSymbols = (chars: string[]) => {
...
const lineCalc = (line: string) => {
  const characters = line.split('')
  let engine: Engine = { number: '', coordinates: [] }
...
const getallCoordinates = (line: string, coordinates: number[]) => {
  const firstCoordinate =
    coordinates[0] - 1 >= 0 ? coordinates[0] - 1 : coordinates[0]
  const finalCoordinate =
    coordinates[coordinates.length - 1] + 2 > line.length
      ? line.length
...
const detectSymbol = (
  ...
  const enginesWithSymbols: Engine[] = []
  engines.forEach((engine) => {
    ...
    if (anySymbolFound) enginesWithSymbols.push(engine)
  })
  return enginesWithSymbols
}

const starCalc = (line: string, lineNumber: number) => {
...
const fillArray = (start: number, finish: number) => {
  const list = []
...
const intersectArrays = (
  ...
  return numberPositions.some((e: number) => starPositions.includes(e))
}

const adyacentNumbers = (
  starPosition: StarPosition,
  currentLine: string,
  numberArrays: lineEngine[]
) => {
  const { position } = starPosition
  const initial = position - 1 >= 0 ? position - 1 : 0
  const final =
    position + 1 > currentLine.length ? currentLine.length : position + 1

  const intersectedNumbersForStar: Engine[] = []
  //engines for this star
  const engines = numberArrays.map((e) => e.engines).flat()
  engines.forEach((elm) => {
    const intersects = intersectArrays(
      elm.coordinates,
      fillArray(initial, final)
    )
    if (intersects) intersectedNumbersForStar.push(elm)
  })
...
function processLines(data: string[]) {
  ...
  //treating all lines
  const enginesWithSymbols: Engine[] = []
  positions.forEach((e: lineEngine) => {
    console.log('analyzing ', e.engines)

    const { lineNumber } = e
    const symbolsDetected = detectSymbol(
      e,
      data[lineNumber],
      data[lineNumber - 1],
      data[lineNumber + 1]
  ...
  const result = enginesWithSymbols.reduce((acc, elm) => {
    ...
    if (isNaN(engineNumber)) return acc
...
function processSecondPart(data: string[]) {
  ...
  const starPositions = data.reduce(
    (acc: StarPosition[], elm: string, ind: number) => {
  ...
  starPositions.forEach((elm) => {
    ...
    const filteredNumberPositions = numberPositions.filter(
      (e) =>
    ...
    const intersectedNumbersForStar = adyacentNumbers(
      elm,
      data[lineNumber],
      filteredNumberPositions
    )

    //we multiply this intersection if it has more than 1 item
    if (intersectedNumbersForStar.length === 2) {
      const power = intersectedNumbersForStar.reduce(
        (acc: number, elm: Engine) => {
          const gear = +elm.number
          if (typeof gear !== 'number') return acc
          return gear * acc
        },
        1
      )
