...
const map: string[][] = data.map((e) => e.split(''))
...
const createSpaceMap = () => {
  ...
  map.forEach((row, ind) => {
    ...
    if (result === 0) addRows.push(ind)
  })
  addRows.forEach((elm, ind) => {
    const newArray: string[] = new Array(newMap[0].length).fill('.')
...
function calculateDistance(starMap: string[][]) {
  const getPositions = starMap.reduce(
    (acc: [number, number][], e: string[], i: number) => {
      const stars = e.forEach((galaxy, pos) => {
        if (galaxy === '#') acc.push([i, pos])
      })
      return acc
    },
    []
...
function calcPath(
  ...
  const vertical = starMap.reduce(
    (acc: string[], elm: string[], ind: number) => {
...
function calculateExpansion(starMap: string[][]) {
  const getPositions = starMap.reduce(
    (acc: [number, number][], e: string[], i: number) => {
      const stars = e.forEach((galaxy, pos) => {
        if (galaxy === '#') acc.push([i, pos])
