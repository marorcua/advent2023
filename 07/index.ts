...
function getHandType(hand: string[]) {
  ...
  const sortedCards = Object.entries(Object.assign({}, { ...counts })).sort(
    ([c, a], [d, b]) => {
      if (a !== b) {
        return b - a
      } else {
...
function getHandTypeWithJoker(hand: string[]) {
  let counts: Hand = {}
  hand.forEach((x) => {
    counts[x] = (counts[x] || 0) + 1
  })

  let sortedCards = Object.entries(Object.assign({}, { ...counts })).sort(
    ([c, a], [d, b]) => {
      if (a !== b) {
        return b - a
      } else {
...
function decideRankByCard(
  ...
  if (handOne.sortedHand[nextCard] && handTwo.sortedHand[nextCard]) {
    if (handOne.sortedHand[nextCard][1] > handTwo.sortedHand[nextCard][1]) {
      ...
      handTwo.sortedHand[nextCard][1] > handOne.sortedHand[nextCard][1]
    ) {
      return -1
    }
  }
  ...
  if (
    -bestCard(
      handOne.originalHand[0][depth],
      handTwo.originalHand[0][depth],
      rank
    ) > 0
  ) {
    //A is greater than B
  ...
  if (!handOne.originalHand[0][depth + 1]) return 0
  return decideRankByCard(handOne, handTwo, depth + 1, rank)
}

function bestCard(cardA: string, cardB: string, rank: number) {
  const rankOrder = [
...
function orderHands(data: { hand: string; points: number }[]) {
  const hands: Position[] = data.map((e, i) => {
    const { hand, points } = e
    return {
...
function orderHandsWithJoker(data: { hand: string; points: number }[]) {
  const hands: Position[] = data.map((e, i) => {
    const { hand, points } = e
    return {
