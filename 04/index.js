...
function secondTest(data) {
    let initialData = data.map((e, i) => ({
        card: i,
        matchings: 1,
    }));
    data.forEach((elem, ind) => {
        const { winningNumbers, listNumbers } = separateLines(elem);
        const matchingNumbers = listNumbers.filter((e) => winningNumbers.indexOf(e) > -1).length;
        ...
        for (let index = 1; index <= matchingNumbers; index++) {
            const nextCardIndex = initialData.findIndex((e) => e.card === index + currentCard.card);
            initialData[nextCardIndex].matchings =
                initialData[nextCardIndex].matchings + currentCard.matchings;
        }
    });
    console.log('final data ', initialData.reduce((acc, elm) => acc + elm.matchings, 0));
}
secondTest(data04);
