...
const lines: string[][] = data.map((e) => e.split(''))

type Coordinate = [number, number]
function searchForStart(arr: string[][]): Coordinate {
  let coordinates: Coordinate = [0, 0]
...
function translateSign(key: string, initial: Coordinate) {
  let go: [Coordinate, Coordinate] = [
    [0, 0],
    [0, 0],
  ]
  switch (key) {
...
const addPositions = (pos1: Coordinate, pos2: Coordinate): Coordinate => {
  const addX = pos1[0] + pos2[0] >= 0 ? pos1[0] + pos2[0] : 0
  const addY = pos1[1] + pos2[1] >= 0 ? pos1[1] + pos2[1] : 0
  return [addX, addY]
}

const findPosition = (map: string[][], coordinate: Coordinate) =>
  map[coordinate[1]][coordinate[0]]

const areEqual = (arr1: Coordinate, arr2: Coordinate) =>
...
function travelPipe(
  ...
  while (
    !areEqual(paths[0][paths[0].length - 1], paths[1][paths[1].length - 1])
  ) {
...
function part1() {
  ...
  const startings: Coordinate[] = [up, down, left, right].filter(
    (e) => !(e[0] === start[0] && e[1] === start[1])
  )

  //of the starting points we get the signs
  const startingSymbols = startings.map((e) => findPosition(lines, e))
  const initialTranslations = startingSymbols.map((e, i) =>
    translateSign(e, startings[i])
  )
  const startingPoints = startings.filter((e, i) =>
    initialTranslations[i].some((elm) => areEqual(elm, start))
  )
